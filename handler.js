...
// ─── Cache del mapa de comandos ───────────────────────────────────────────────
// (index.js lo recarga al detectar cambios en la carpeta plugins/)
let _cachedCmdMap = null
let _cachedPluginsRef = null

function getCmdMap(plugins) {
    if (_cachedCmdMap && _cachedPluginsRef === plugins) return _cachedCmdMap
    _cachedCmdMap = buildCmdMap(plugins)
    _cachedPluginsRef = plugins
    return _cachedCmdMap
}
...
for (const [, p] of plugins) {
    ...
    try {
        ...
        // ── PARSEO ────────────────────────────────────────────────────────────
        const cmdMap = getCmdMap(plugins)
