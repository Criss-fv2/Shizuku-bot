...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    if (!searchQuery) {
        return m.reply(
            ...
            `📌 Uso: *${prefix}${command} <búsqueda>*\n` +
            `💡 Ejemplo: *${prefix}${command} dark aesthetic*\n\n` +
    ...
    try {
        ...
        if (!global.pinterestOffsets[cacheKey]) global.pinterestOffsets[cacheKey] = 0
        ...
        if (endIndex > searchResults.length) {
            startIndex = 0
            endIndex   = 13
            global.pinterestOffsets[cacheKey] = 13
        } else {
            global.pinterestOffsets[cacheKey] = endIndex
        }

        const urls = searchResults.slice(startIndex, endIndex)

        // Descargar buffers en paralelo
        ...
        // sendAlbumMessage → envía TODO como un solo mensaje de álbum atómico
        // Si el fork no lo tiene, fallback a envío paralelo con quoted en el primero
        if (typeof conn.sendAlbumMessage === 'function') {
            await conn.sendAlbumMessage(
                m.chat,
                valid.map(buf => ({ image: buf, mimetype: 'image/jpeg' })),
                { quoted: m }
            )
        } else {
            // Fallback: envío paralelo — WA los agrupa si llegan juntos
            await Promise.all(
                valid.map(buf =>
                    conn.sendMessage(m.chat, { image: buf, mimetype: 'image/jpeg' })
                )
            )
        }
