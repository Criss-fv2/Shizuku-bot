...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    ...
    try {
        let results = await searchPinterest(searchQuery)
        ...
        let start = global.pinterestOffsets[cacheKey]
        let end   = start + 13
        if (end > results.length) { start = 0; end = 13 }
        global.pinterestOffsets[cacheKey] = end

        const urls = results.slice(start, end)

      
        const buffers = (await Promise.all(
            urls.map(url =>
                axios.get(url, { responseType: 'arraybuffer', timeout: 12000 })
        ...
        await conn.sendAlbumMessage(
            m.chat,
            buffers.map(buf => ({ image: buf, mimetype: 'image/jpeg' })),
            { quoted: m }
        )
