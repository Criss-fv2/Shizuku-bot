// ═════════════════════════════════════════════════════════════════
// COMANDO: .work (Trabajar y ganar dinero - 5 MINUTOS)
...
// 🕷️ 100 Frases de trabajo: Normales, Robos, Prostitución y Estafas
const frasesTrabajo = [
...
const handler = async (m, { conn }) => {
    // ─── PASO 1: Verificar y crear usuario ─────────────────────────
    ...
    // ─── PASO 2: Verificar tiempo de espera (Cooldown) ─────────────
    ...
    if (tiempoTranscurrido < cooldownTime && user.eco.lastwork !== 0) {
        ...
        const minutos = Math.floor(tiempoFaltante / 60000)
        const segundos = Math.floor((tiempoFaltante % 60000) / 1000)
        
        return m.reply(`⏳ Ya trabajaste mucho, fiera. Descansa *${minutos}m ${segundos}s* antes de volver a la calle. 🕷️`)
    }

    // ─── PASO 3: Ejecutar trabajo y generar ganancias ──────────────
    const trabajoAleatorio = frasesTrabajo[Math.floor(Math.random() * frasesTrabajo.length)]
    
    // Generar dinero al azar entre 150 y 500 Nen
    const dineroGanado = Math.floor(Math.random() * (500 - 150 + 1)) + 150
    
    // Generar experiencia basada en el dinero ganado (10%)
    const expGanada = Math.floor(dineroGanado / 10)

    // ─── PASO 4: Actualizar datos de economía ──────────────────────
    user.eco.dinero += dineroGanado
    user.eco.exp += expGanada
    user.eco.trabajos_realizados += 1
    user.eco.lastwork = ahora
    
    // Calcular subida de nivel (cada 1000 EXP sube de nivel)
...
_...dinero sucio transferido, vuelve en 5 minutos._ 🖤
    `
