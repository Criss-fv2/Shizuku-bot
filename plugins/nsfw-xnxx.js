...
import { database } from '../lib/database.js' // 🕷️ Importación clave para leer tu base de datos

const handler = async (msg, { conn, args, usedPrefix, command }) => {
  // 🕷️ Verificación exacta tomada de tu comando funcional
  if (!database.data.groups?.[msg.chat]?.nsfw) {
    return conn.sendMessage(msg.chat, { text: '🚫 El contenido NSFW está desactivado en este grupo.\n\nUn admin puede activarlo con *#nable nsfw on*' }, { quoted: msg })
  }
  ...
  try {
    ...
    if (!data.length) {
      await msg.react('❌')
      return conn.sendMessage(msg.chat, { text: `🖤 Sin resultados en la red para: ${query}` }, { quoted: msg })
    }

    // Guardar resultados en la memoria caché temporal
    ...
    text += `🕷️ *Responde a este mensaje con un número del 1 al 10 para descargar el video.*`
    ...
    console.error(e)
    await msg.react('❌')
    return conn.sendMessage(msg.chat, { text: `❌ *Error interno:* No se pudo procesar la solicitud.` }, { quoted: msg })
  }
}
...
handler.before = async (msg, { conn }) => {
  ...
  if (!msg.quoted.text || !msg.quoted.text.includes('🖤 *𝑺𝒉𝒊𝖟𝒖𝒌𝒖 𝕾𝖞𝖘𝒕𝒆𝒎* 🖤')) return
  ...
  try {
    ...
    if (!res || !res.low) throw new Error('Error de extracción de servidor.')
    ...
    await msg.react('🖤')
    delete conn.xnxxCache[msg.sender] // Limpia la caché tras la descarga exitosa
  } catch (e) {
    console.error(e)
    await msg.react('❌')
    return conn.sendMessage(msg.chat, { text: `❌ *Fallo de servidor:* No se pudo extraer el archivo de video.` }, { quoted: msg })
  }
}
